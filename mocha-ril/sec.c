...
void ipc2ril_card_status(struct ipc_sec_pin_status_response *pin_status, RIL_CardStatus *card_status)
{
	...
	ALOGD("Selecting application #%d on %d", (int) sim_status, app_status_array_length);
}

void ril_tokens_pin_status_dump(void)
{
	ALOGD("ril_tokens_pin_status_dump:\n\
...
void ipc_sec_pin_status(struct ipc_message_info *info)
{
	...
	switch(info->type) {
		case IPC_TYPE_NOTI:
			...
			break;
		case IPC_TYPE_RESP:
			ALOGD("Got SOL PIN status message");

			if(ril_state.tokens.pin_status != t)
				ALOGE("PIN status tokens mismatch");
			...
			break;
		default:
			ALOGE("%s: unhandled ipc method: %d", __FUNCTION__, info->type);
			break;
	}
...
void ril_request_get_sim_status(RIL_Token t)
{
	...
	if(ril_state.tokens.pin_status == RIL_TOKEN_DATA_WAITING) {
		...
		ALOGE("Another request is going on, returning UNSOL data");
...
void ipc_sec_pin_status_complete(struct ipc_message_info *info)
{
	...
	if(rc < 0) {
		if((phone_res->code & 0x00ff) == 0x10) {
			...
			ALOGE("Wrong password and no attempts left!");
			...
			ALOGE("There was an error during pin status complete!");
